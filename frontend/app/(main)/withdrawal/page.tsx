...
const WithdrawalPage = () => {
  ...
  const transactions = [
    { id: "TXN-001", date: "Oct 24, 2023", amount: 500.00, status: "completed", method: "Bank Transfer" },
    { id: "TXN-002", date: "Oct 20, 2023", amount: 1200.00, status: "pending", method: "PayPal" },
    { id: "TXN-003", date: "Oct 15, 2023", amount: 350.50, status: "completed", method: "Bank Transfer" },
    { id: "TXN-004", date: "Oct 08, 2023", amount: 2100.00, status: "failed", method: "Bank Transfer" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
          ...
          <div className="relative z-10">
            ...
            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-sm font-medium opacity-80 ml-2">Withdrawal Amount</label>
                <div className="relative">
                  ...
                  <input 
                    ...
                    className="w-full bg-white/10 border border-white/20 rounded-2xl py-5 pl-12 pr-6 text-3xl font-black placeholder:text-white/30 focus:outline-none focus:bg-white/20 transition-all"
                    placeholder="0.00"
                  />
                </div>
              </div>
              ...
              <div className="bg-white/10 p-4 rounded-2xl border border-white/10">
                <p className="text-xs opacity-70 mb-2 font-medium">Payout Method</p>
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <div className="bg-white/10 p-2 rounded-lg">
                      <CreditCard className="w-4 h-4" />
                    </div>
                    <span className="text-sm font-bold">Bank ending in 4242</span>
                  </div>
                  <Button variant="ghost" size="sm" className="text-xs h-8 hover:bg-white/20 text-white">Change</Button>
                </div>
              </div>

              <Button className="w-full py-7 rounded-2xl bg-white text-primary hover:bg-white/90 font-black text-lg transition-transform active:scale-95 shadow-xl shadow-black/10">
                Initiate Transfer
              </Button>
              
              <p className="text-[10px] text-center opacity-60">Estimated arrival: 2-3 business days. Min. withdrawal $50.00</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <div className="p-8 border-b border-border flex justify-between items-center bg-muted/20">
            ...
            <Button variant="outline" size="sm" className="rounded-xl gap-2 font-bold text-xs h-10 border-border">
                ...
                {transactions.map((txn) => (
                  <tr key={txn.id} className="group hover:bg-muted/30 transition-colors">
                    <td className="px-8 py-6">
                      ...
                      <p className="text-[10px] text-muted-foreground font-medium">{txn.method}</p>
                    </td>
                    <td className="px-8 py-6">
          ...
          <div className="p-6 text-center bg-muted/10 border-t border-border/50 rounded-b-[2.5rem]">
            <Button variant="ghost" className="text-xs font-bold text-primary">Download Financial Report (.PDF)</Button>
          </div>
        </motion.div>
      </div>
    </div>
