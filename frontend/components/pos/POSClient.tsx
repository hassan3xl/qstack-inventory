...
import { useCreateSale } from "@/lib/hooks/sales.hook";
...
export default function POSClient() {
  const { data: products, isLoading: productsLoading, refetch } = useGetProducts();
  ...
  const handleBarcodeSubmit = (e: React.FormEvent) => {
    ...
    const targetProduct = products.find(
      (p: any) => p.inventory?.sku && p.inventory.sku.toLowerCase() === barcodeInput.trim().toLowerCase()
    );
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      const errMsg = err?.response?.data?.non_field_errors?.[0] || err?.message || "Failed to process sale";
      addToast({
    ...
    <div className="space-y-6">
        ...
        <form onSubmit={handleBarcodeSubmit} className="flex items-center gap-2 w-full md:w-auto">
          <div className="relative flex-1 md:w-72">
            ...
            <input
              ...
              className="w-full pl-11 pr-4 py-3 bg-muted/40 border border-border rounded-2xl focus:bg-background focus:ring-2 focus:ring-primary/20 transition-all text-sm font-semibold"
            />
          </div>
          <Button type="submit" className="rounded-2xl h-11 px-5 font-bold shadow-md shadow-primary/10">
            Scan
          </Button>
        </form>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="xl:col-span-2 space-y-6">
          ...
          <div className="bg-card p-4 rounded-3xl border border-border/50 flex flex-wrap gap-2 items-center">
            <Button
            ...
            {categories.map((cat) => (
              <Button
                ...
                className="rounded-2xl text-xs font-bold capitalize"
              >
                {cat}
              </Button>
            ))}
            ...
            <div className="relative ml-auto w-full md:w-60 mt-2 md:mt-0">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-muted/30 border border-transparent rounded-xl focus:bg-background focus:border-border transition-all text-xs font-semibold"
              />
            </div>
          </div>
          ...
          {productsLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 bg-card border border-dashed rounded-[2rem]">
              ...
              <p className="text-xs text-muted-foreground/80 mt-1">Try refining your search keyword or classification filters.</p>
            </div>
          ) : (
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              {filteredProducts.map((product: any) => {
                ...
                const pImage = product.images && product.images.length > 0 ? product.images[0].image : "/makeup_product.png";

                return (
                  <Card 
                    key={product.id}
                    onClick={() => !isOutOfStock && addToCart(product)}
                    className={`rounded-2xl overflow-hidden border border-border/50 hover:shadow-lg transition-all duration-300 cursor-pointer group relative ${
                      isOutOfStock ? "opacity-60 bg-muted/30 pointer-events-none" : ""
                    }`}
                  >
                      ...
                      <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-border/30">
                        ...
                        <div className="flex flex-col items-end">
                          ...
                          <span className={`text-[10px] font-black mt-0.5 ${
                            product.stock <= 5 ? "text-orange-500 animate-pulse" : "text-emerald-600 dark:text-emerald-400"
                          }`}>
          ...
          <Card className="rounded-[2rem] border-primary/10 shadow-xl shadow-primary/5 overflow-hidden sticky top-6">
            <CardHeader className="p-6 border-b border-border/50 flex flex-row items-center justify-between">
              <CardTitle className="text-lg font-black flex items-center gap-2">
                ...
                Active Receipt Cart ({cart.reduce((sum, i) => sum + i.quantity, 0)})
              </CardTitle>
              {cart.length > 0 && (
                <Button 
                  variant="ghost" 
                  size="sm" 
                  onClick={() => setCart([])} 
                  className="text-red-500 hover:text-red-700 hover:bg-red-50 text-xs font-bold rounded-xl"
                >
                  Clear Cart
                </Button>
              )}
            ...
            <CardContent className="p-0">
              {cart.length === 0 ? (
                <div className="text-center py-20 px-6">
                  ...
                  <p className="text-xs text-muted-foreground/80 mt-1">Select items from the catalog or scan barcode SKU above.</p>
                </div>
              ) : (
                    ...
                    {cart.map((item) => (
                      <div key={item.id} className="py-3 flex items-center justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <h4 className="font-bold text-xs leading-tight text-foreground truncate">{item.name}</h4>
                          <span className="text-[10px] text-muted-foreground">Unit: {formatNaira(item.unit_price)}</span>
                        </div>
                  ...
                  <div className="bg-muted/30 p-6 border-t border-border/50 space-y-4">
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        <button
                          ...
                          className={`py-2 px-3 rounded-xl border font-bold text-xs flex flex-col items-center justify-center gap-1.5 transition-all ${
                            paymentMethod === "cash" 
                    ...
                    <div className="pt-4 border-t border-border/50 space-y-2">
                      ...
                      <div className="flex justify-between items-center pt-2 border-t border-border/50">
                        <span className="font-black text-sm text-foreground">Grand Total</span>
                    ...
                    <Button
                      ...
                      className="w-full rounded-2xl h-12 font-black shadow-xl shadow-primary/20 active:scale-95 transition-all flex items-center justify-center gap-2 mt-4"
                    >
                      {createSaleMutation.isPending ? "Processing..." : "Complete Checkout (Print Receipt)"}
                    </Button>
                  </div>
                </div>
              )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-xs p-4">
          <Card className="w-full max-w-md rounded-3xl overflow-hidden shadow-2xl border border-border/50">
            <div className="bg-primary/5 p-6 border-b border-border/50 text-center relative">
              ...
              <CardTitle className="text-xl font-black">Transaction Receipt</CardTitle>
              <p className="text-xs text-muted-foreground mt-1">Transaction logged & receipt ready for printing.</p>
            </div>
            ...
            <CardContent className="p-6 space-y-6">
              ...
              <div className="space-y-1.5 text-xs border-b border-dashed border-border pb-4">
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span className="font-bold text-foreground">{completedSale.sale_number}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                ...
                <div className="flex justify-between text-muted-foreground">
                  ...
                  <span className="font-semibold text-foreground">{completedSale.cashier_email || "System"}</span>
                </div>
                <div className="flex justify-between text-muted-foreground">
                  <span>Payment Mode:</span>
                  <span className="font-bold capitalize text-primary">{completedSale.payment_method.replace('_', ' ')}</span>
                </div>
              </div>
              ...
              <div className="border-t border-border pt-4 space-y-2 text-xs">
                ...
                <div className="flex justify-between items-center pt-2 border-t border-dashed border-border font-black text-sm">
                  ...
                  <span className="text-primary text-base">{formatNaira(completedSale.total_amount)}</span>
                </div>
              </div>
